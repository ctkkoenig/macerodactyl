...
public final class StatsCoordinator {
    ...
    private func refreshSnapshot() async {
        guard let cli = cli() else { snapshotAvailable = false; return }
    ...
    public func focus(containerID: String, containerName: String, running: Bool) {
        ...
        history = []
        focused = nil
        guard let cli = cli() else { focusedAvailability = .unavailable("Docker isn’t available."); return }
        guard running else { focusedAvailability = .unavailable("Container isn’t running."); return }
