        ...
        if env.daemon == .daemonDown {
            return [
                StartupAdvisory(
                    ...
                    remedy:
                        "Start Docker Desktop (or your Docker provider) and wait for it to finish launching. Macerodactyl never starts the daemon itself."
                )
            ]
        }
        ...
        if !env.perlAvailable {
            advisories.append(
                StartupAdvisory(
                    ...
                    detail:
                        "Scheduled restarts normally run under /usr/bin/perl, which enforces a hard timeout so a stuck docker can’t hang forever.",
                    remedy:
                        "Schedules still work, but a hung restart won’t be force-killed at the deadline. Restoring /usr/bin/perl re-enables the safeguard."
                ))
        }
        if !env.stacksRootExists {
            advisories.append(
                StartupAdvisory(
                    ...
                    detail:
                        "\(env.stacksRootPath) isn’t there. File editing is scoped to stacks under this folder, so until it exists (and holds your compose projects) the Files tab stays empty.",
        ...
        if env.containerCount == 0 {
            advisories.append(
                StartupAdvisory(
                    ...
                    detail:
                        "Docker is running but nothing is defined. Compose stacks and bare `docker run` containers both appear here once they exist.",
